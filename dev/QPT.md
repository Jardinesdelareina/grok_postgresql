...
### Изменение структуры SQL-запроса для оптимизации производительности

Порядок выполнения запроса пронумерован:
```sql
SELECT
p.email,                                  -- 6. Указание колонок, которые будут фигурировать в выборке
COUNT(o.order_id) as total_orders,        --
SUM(o.order_amount) as total_amount       --
FROM profiles p                           -- 1. Откуда берутся данные
JOIN orders o ON p.id = o.profile_id      -- 2. Объединение таблиц-источников данных
WHERE o.order_date >= '2023-01-01'        -- 3. Условие фильтрации к объединенной таблице
GROUP BY p.email                          -- 4. Группировка данных по указанным колонкам
HAVING total_amount >= 1000               -- 5. Фильтрация групп строк по условию ("постфильтрация")
ORDER BY p.email                          -- 7. Сортировка строк по указанным колонкам
LIMIT 100;                                -- 8. Ограничение выборки по количеству строк
```

<b>SAGRABLE</b> - (Search ARGument ABLE) - это термин, который используется в SQL для описания запросов, которые позволяют использовать индексы для оптимизации поиска данных. SARGABLE-запросы могут быть выполнены эффективно, так как они позволяют базе данных применять индексы к условиям поиска, что уменьшает количество строк, которые нужно обработать.

Использование операторов равенства (`=`), неравенства (`<>`, `!=`), диапазона (`BETWEEN`) или членства (`IN`) в индексированных столбцах может сделать запрос SARGABLE.

Использование операторов отрицания (`NOT`), подстановочных знаков (`LIKE`) или арифметических операций (`+`, `-`, `*`, `/`) в индексированных столбцах может сделать запрос не SARGABLE.

Для написания SAGRABLE-запроса нужно:

* Избегать использования функций для индексированных столбцов в предложении WHERE, таких, как UPPER(), LOWER(), SUBSTRING() и т.д.
* Избегать использования арифметических операций над индексированными столбцами в предложении WHERE, например `column + 1 > 10`, `column * 2 < 20` и т.д.
* Избегать использования операторов отрицания для индексированных столбцов в предложении WHERE, таких, как NOT IN, NOT LIKE, NOT EXISTS и т.д.
* Избегать использования подстановочных знаков в индексированных столбцах в предложении WHERE с ведущими подстановочными знаками (%), такими как LIKE '%abc', LIKE '-xyz%' и т.д.
* Использовать соответствующие типы данных для столбцов и литералов, чтобы избежать неявных преобразований, которые могут повлиять на использование индекса.

Пример SARGABLE запроса:
```sql
SELECT * FROM table_name WHERE indexed_column = 10;
```

Пример не SARGABLE запроса:
```sql
SELECT * FROM table_name WHERE UPPER(column_name) = 'VALUE';
```

В примере не SARGABLE запроса вызов функции `UPPER` делает условие `несинтаксическим`, что препятствует использованию индексов.

Рекомендации для повышения производительности SQL-запросов:
1. Индексация часто используемых колонок,
2. JOIN'ы в правильном порядке, чтобы минимизиовать размер промежуточного результата,
3. Ограничение итогово выборки (LIMIT),
4. Использование сортировки и группировки только когда это действительно необходимо, 
5. Применение WHERE как можно раньше, так как этот фильтр уменьшает количество строк,
6. Использование рационально-правильных типов данных на колонках таблиц,
7. Избежание математических действий и функций в таблицах.


### Влияние порядка колонок таблицы на эффективность хранения данных

В PostgreSQL строка в таблице имеет минимальный размер в 24 байта. Это связано с тем, что в строке содержится заголовок с различной служебной информацией, типа номера транзакции, ID самой строки и т.д.
...
Для компьютерных систем проще находить данные на диске когда они выровнены по определенной границе. Это влечет за собой то, что между данными нужно вставлять разделители, чтобы они были выровнены относительно друг друга на диске.

Можно посмотреть какое выравнивание для каких типов данных использует PostgreSQL, обратившись к системному каталогу pg_type, который содержит информацию об используемых типах данных:
...
Типы выравнивания в PostgreSQL:
* c - выравнивание не нужно;
* s- короткое выравнивание (2 байта);
* i - int выравнивание (4 байта);
* d - двойное выравнивание (8 байтов).

Выравнивание применяется и к индексам. 
Основная рекомендация - это располагать колонки таблиц от большего типа данных (большего по размеру на диске) к меньшему. Такой приём естественным образом выравнивает данные на диске и требуется меньше заполнителей между данными, а следовательно и места на физических дисках.

Проверка текущего порядка колонок в таблице:
```sql
SELECT a.attname, t.typname, t.typalign, t.typlen
FROM pg_class c
JOIN pg_attribute a ON (a.attrelid = c.oid)
JOIN pg_type t ON (t.oid = a.atttypid)
WHERE c.relname = '<название_таблицы>' AND a.attnum >= 0
ORDER BY a.attnum;
```

Нужный порядок колонок в таблице:
```sql
SELECT a.attname, t.typname, t.typalign, t.typlen
FROM pg_class c
JOIN pg_attribute a ON (a.attrelid = c.oid)
