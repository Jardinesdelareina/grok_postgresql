...
* Дедубликация данных: при попытке вставить дублирующуюся строку, можно указать действие для обновления значений существующей строки или пропуска вставки.
...
1. Пример использования `ON CONFLICT DO NOTHING` для пропуска вставки, если запись уже существует:
```sql
...
В этом случае, если запись с `id=1` уже существует, то новая запись не будет вставлена.
2. Пример использования `ON CONFLICT DO UPDATE` для обновления существующей записи:
...
Если запись с `id=1` уже существует, то поле `name` будет обновлено на 'Bob'.

3. Пример с условием WHERE для более точного определения случаев обновления:
```sql
...
DO UPDATE SET 
    name = EXCLUDED.name,
    age = EXCLUDED.age
WHERE table_name.age < 30;
```

В этом примере запись с `id=1` будет обновлена только если ее возраст меньше 30.
...
Параметр UNIQUE при создании индекса означает, что значения в столбце или комбинации столбцов, для которых создается индекс, должны быть уникальными. То есть, индекс не позволит добавить или обновить запись, если такие значения уже существуют в таблице.


#### CONCURRENTLY

Параметр `CONCURRENTLY` при создании индекса означает, что индекс будет строиться без блокировки таблицы. Это позволяет выполнять создание индекса параллельно с операциями записи (INSERT, UPDATE, DELETE) на таблицу, не блокируя доступ к данным для других пользователей.
...
Таким образом, функция `RANK()` присваивает ранг продуктам в порядке возрастания цены.
...
Функция split_part используется для разделения строки на подстроки с использованием определенного разделителя и возврата подстроки с указанным индексом. 

`split_part(string, delimiter, index)`

где:

<b>string</b> - исходная строка, которую нужно разделить
<b>delimiter</b> - разделитель, по которому будет произведено разделение строки
<b>index</b> - индекс подстроки, которую нужно вернуть

```sql
SELECT split_part('abc.def.ghi', '.', 2);
```

Результат выполнения запроса будет 'def', так как при указании индекса 2 была выбрана подстрока 'def' из исходной строки 'abc.def.ghi', разделенной по символу '.'.


### substring

Функция `substring` возвращает подстроку из заданной строки.
...
Функция LEAD работает аналогично, но возвращает значение следующей строки.


### GROUPING SETS

GROUPING SETS — это функция, которая позволяет создавать несколько уровней агрегирования в одном запросе. Она используется в сочетании с оператором GROUP BY и позволяет группировать данные по разным наборам колонок.

Обычно, когда используеся `GROUP BY`, данные группируются по одному набору столбцов и агрегируются значения (например, используя функции SUM, COUNT и т. д.).
С помощью `GROUPING SETS`  можно указать несколько наборов группировки в одном запросе. Это позволяет получать результаты для разных комбинаций столбцов в одной выборке.

Запрос с GROUPING SETS к таблице 
...
GROUP BY GROUPING SETS (
    (product, region),  -- Сумма по продукту и региону
    (product),          -- Сумма по продукту
    (region),           -- Сумма по региону
    ()                  -- Общая сумма
);
```
...
* Строки с конкретными значениями product и region показывают сумму продаж для каждой комбинации.
* Строки с NULL в одной из колонок показывают агрегированные значения по оставшимся колонкам.
* Строка с двумя NULL показывает общую сумму всех продаж.