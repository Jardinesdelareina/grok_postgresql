...
Таблица представляет собой хранилице исторических данных о ценовых изменениях криптовалют. Данные загружаются порциями - каждую минуту по каждому тикеру.

|FK|PK|Column|Type|Length|Unique|Nullable|Description|
|---|---|---|---|---|---|---|---|
|||m_symbol|valid_symbol|||*|Название тикера|
...
Таблица представляет собой хранилище данных о завершенных транзакциях на бирже MEXC. Сделки на открытом рынке, поступающие в реальном времени через API. Таблица партиционируется по списку тикеров.

|FK|PK|Column|Type|Length|Unique|Nullable|Description|
|---|---|---|---|---|---|---|---|
|||d_symbol|valid_symbol|||*|Название тикера|
|||d_time|timestamptz|||*|Время записи|
|||d_side|varchar|4||*|Тип ордера|
|||d_price|real|||*|Цена ордера|
|||d_qty|real|||*|Объем ордера|
