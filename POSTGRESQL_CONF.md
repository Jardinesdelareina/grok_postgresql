...
`data_directory = '/var/lib/postgresql/14/main'`	использовать данные в другом каталоге

`hba_file = '/etc/postgresql/14/main/pg_hba.conf'`	файл аутентификации на основе хоста

`ident_file = '/etc/postgresql/14/main/pg_ident.conf'`	файл конфигурации идентификатора

<em>Если external_pid_file не задан явно, дополнительный файл PID не записывается.</em>
`external_pid_file = '/var/run/postgresql/14-main.pid'`	написать дополнительный файл PID

<em>Изменения требуют перезапуска сервера</em>
...
`#listen_addresses = 'localhost'`	какой IP-адрес(а) прослушивать (список адресов, разделенный запятыми, по умолчанию — «localhost», используйте '*' для всех)

`port = 5432`	порт

`max_connections = 100`	число клиентов, которые могут подсоединяться к базе данных

`#superuser_reserved_connections = 3`	число соединений, которые зарезервированы для суперпользователя

`unix_socket_directories = '/var/run/postgresql'`	список каталогов, разделенный запятыми
